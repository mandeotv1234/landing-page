...
import { ChevronDown, ChevronUp, Zap, Settings, Trash2 } from 'lucide-react'
...
export default function EventLog({ events, pixelId, setPixelId }) {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 w-80 shadow-2xl rounded-2xl overflow-hidden border border-slate-200 bg-white">
      {/* Header */}
        ...
        <div className="border-b border-slate-200 p-3 bg-slate-50 text-xs animate-fade-in">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 border border-slate-300 rounded-lg px-2 py-1.5 text-xs font-mono outline-none focus:ring-1 focus:ring-blue-500"
            />
            <button onClick={savePixelId} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-semibold">Lưu</button>
          </div>
          <div className="text-slate-400 mt-1">Active: <span className="font-mono text-slate-600">{pixelId}</span></div>
        </div>
      )}
        ...
        <div className="max-h-80 overflow-y-auto">
          {events.length === 0 ? (
            <div className="text-center text-slate-400 py-8 text-sm">
              ...
              Chưa có event nào được gửi.<br />Hãy tương tác với trang!
            </div>
          ) : events.map((e, idx) => (
            <div key={idx} className="border-b border-slate-100 last:border-0">
              ...
              >
                <div className="flex items-center gap-2">
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${EVENT_COLORS[e.name] || 'bg-slate-100 text-slate-600'}`}>
                    {e.name}
                  </span>
                </div>
              ...
              {expandedIdx === idx && Object.keys(e.params).length > 0 && (
                <div className="px-4 pb-3 animate-fade-in">
                  <pre className="text-xs bg-slate-50 rounded-lg p-2 overflow-x-auto text-slate-600 max-h-32">
